...
public class mapper extends MapReduceBase implements Mapper<Object, Text, Text, Text> {

	/*public void map(WritableComparable key, Writable values,
			OutputCollector output, Reporter reporter) throws IOException {
	}*/
	private static int[][] map_cost = {{0, 81, 37, 64, 70, 83, 36, 57, 92, 84, 9, 37, 37, 79, 21, 25, 25, 92, 11, 17, 3, 30, 63, 65, 63, 40, 30, 5, 65, 2, 31, 14, 17, 22, 34, 18, 24, 100, 52, 22, 10, 69, 34, 26, 88, 15, 11, 28, 31, 29, },
	...
	private static int[] map_w = {33, 81, 63, 99, 80, 94, 62, 27, 35, 87, 39, 46, 22, 11, 61, 1, 46, 93, 73, 5, 90, 82, 79, 96, 36, 38, 83, 93, 5, 12, 18,98, 46, 13, 97, 45, 24, 95, 33, 97, 55, 62, 44, 34, 27, 83, 3, 16, 59, 27};
	private static int weight = 0, cost = 0;
	...
	public void map(Object key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
		...
		int sub_int1 = Integer.parseInt(sub_str[1]);
		int sub_int2 = Integer.parseInt(sub_str[2]);
		...
		for( int i = 1; i <= POIsize; i++ ){
			...
			if( map_cost[path_int-1][i-1] == 0 || map_cost[i-1][path_int-1] == 0)
				...
				sort[path_str.length] = i;
			}
			
			//if isConnected (P, i) and !P.contains(i) then
			if( isconnected && !iscontains ){
				//Path newPath = P.append(i)
				new_path = sub_str[0]+":"+i;
				...
				if( cost <= 60 ){
					String new_w = new_path+"\t"+cost+"\t"+weight;
					String new_k = "";
					//Key newKey = parsePath(newPath).sort
					//sort
					for( int si  = 0; si < sort.length-1; si++ ){
						for( int sj  = 0; sj < sort.length-1-si; sj++ ){
							if(sort[sj] > sort[sj+1]){
								int tmp = sort[sj];
								sort[sj] = sort[sj+1];
								sort[sj+1] = tmp;
							}
					...
					//context.collect(newKey, newPath)
					outkey.set(new_k);
					outvalue.set(new_w);
